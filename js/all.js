...
/* ---- data/1FiSxj2yDPeGuuf6iBwRAXvEMQJATAZNt6/js/lib/byte_size.js ---- */
...
/* ---- data/1FiSxj2yDPeGuuf6iBwRAXvEMQJATAZNt6/js/lib/extend.js ---- */
...
/* ---- data/1FiSxj2yDPeGuuf6iBwRAXvEMQJATAZNt6/js/lib/frame.coffee ---- */
...
(function() {
  ...
  ZeroFrame = (function(superClass) {
    ...
    ZeroFrame.prototype.onRequest = function(cmd, message) {
      return this.log("Unknown request " + cmd, message);
    };
...
/* ---- data/1FiSxj2yDPeGuuf6iBwRAXvEMQJATAZNt6/js/lib/md5.js ---- */
...
/* ---- data/1FiSxj2yDPeGuuf6iBwRAXvEMQJATAZNt6/js/lib/mustache.js ---- */
...
/* ---- data/1FiSxj2yDPeGuuf6iBwRAXvEMQJATAZNt6/js/lib/promise-done.js ---- */
...
/* ---- data/1FiSxj2yDPeGuuf6iBwRAXvEMQJATAZNt6/js/lib/time.coffee ---- */
...
/* ---- data/1FiSxj2yDPeGuuf6iBwRAXvEMQJATAZNt6/js/lib/url_regexp.js ---- */


window.urlRegexp = /((([A-Za-z]{3,9}:(?:\/\/)?)(?:[-;:&=\+\$,\w]+@)?[A-Za-z0-9.-]+|(?:www.|[-;:&=\+\$,\w]+@)[A-Za-z0-9.-]+)((\:\d+)?(?:\/[\+~%\/.\w-_]*)?\??(?:[-\+=&;%@.\w_\#\/\?\*\:]*))?)/g;


/* ---- data/1FiSxj2yDPeGuuf6iBwRAXvEMQJATAZNt6/js/zengine/board_list.coffee ---- */
...
(function() {
  ...
  BoardList = (function() {
    ...
    BoardList.prototype.reloadBoardList = function(forceReload) {
      if (forceReload == null) {
        forceReload = false;
      }
    ...
    BoardList.prototype.renderMainPageBoardList = function() {
      return new Promise((function(_this) {
        return function(fulfill, reject) {
          return _this.reloadBoardList().then(function() {
            var container;
...
/* ---- data/1FiSxj2yDPeGuuf6iBwRAXvEMQJATAZNt6/js/zengine/forms.coffee ---- */
      ...
      return new Promise((function(_this) {
        return function(fulfill, reject) {
          ...
          return Nullchan.cmd("fileGet", {
            ...
            formData.board = Nullchan.currentBoard.abbr;
...
/* ---- data/1FiSxj2yDPeGuuf6iBwRAXvEMQJATAZNt6/js/zengine/markup.coffee ---- */
...
/* ---- data/1FiSxj2yDPeGuuf6iBwRAXvEMQJATAZNt6/js/zengine/threads.coffee ---- */
  ...
  Threads = (function() {
      ...
      return new Promise((function(_this) {
        return function(fulfill, reject) {
          return _this.loadThreads().then(function(threads) {
            var cnt, i, len, node, ref, thread;
            _this.container.innerHTML = "";
            _this.drawButton("start new thread");
            if (threads.length === 0) {
              node = Templates.wrapNode(Templates.render("empty-board"));
    ...
    Threads.prototype.loadThreads = function() {
      return new Promise((function(_this) {
        return function(fulfill, reject) {
          var query;
          query = "SELECT message.*, keyvalue.value AS cert_user_id FROM message\nLEFT JOIN json AS data_json USING (json_id)\nLEFT JOIN json AS content_json ON (\n  data_json.directory = content_json.directory AND content_json.file_name = 'content.json'\n)\nLEFT JOIN keyvalue ON (keyvalue.key = 'cert_user_id' AND keyvalue.json_id = content_json.json_id)\nWHERE message.board = '" + Nullchan.currentBoard.abbr + "'";
          return Nullchan.cmd("dbQuery", query, function(data) {
...
/* ---- data/1FiSxj2yDPeGuuf6iBwRAXvEMQJATAZNt6/js/zengine/z.coffee ---- */
...
(function() {
  ...
  Nullchan = (function(superClass) {
    ...
    function Nullchan() {
      ...
      this.showMainPage = bind(this.showMainPage, this);
    ...
    Nullchan.prototype.currentBoard = null;

    Nullchan.prototype.optionalIsSet = false;

    Nullchan.prototype.currentPath = [];
    ...
    Nullchan.prototype.onOpenWebsocket = function(event) {
      if (!this.initialized) {
        ...
        return this.loadSiteInfo().then((function(_this) {
    ...
    Nullchan.prototype.updateHeader = function() {
      return Header.update(this.siteInfo, this.settings);
    };
    ...
    Nullchan.prototype.determineRoute = function() {
      return BoardList.reloadBoardList().then((function(_this) {
        return function() {
          var board, promise;
          _this.grepPath();
          promise = null;
          _this.log(_this.currentPath);
          if (_this.currentPath.length === 0) {
            _this.page = "main";
            promise = _this.showMainPage();
          } else if ((board = BoardList.get(_this.currentPath[0])) !== null) {
            _this.currentBoard = board;
            if (_this.currentPath.length === 1) {
              _this.page = "list";
              promise = Threads.showList();
            } else if (_this.currentPath[1] === "thread") {
              _this.page = "thread";
              promise = Threads.showThread(_this.currentPath[2]);
            }
          }
          if (promise === null) {
            _this.renderNotFound();
            _this.togglePreloader(false);
            return;
          }
    ...
    Nullchan.prototype.grepPath = function() {
      var i, len, part, ref, result;
      result = [];
      ...
      return this.currentPath = result;
    };

    Nullchan.prototype.showMainPage = function() {
      return new Promise((function(_this) {
        return function(fulfill, reject) {
          _this.container.innerHTML = Templates.render("mainpage");
          _this.updateHeader();
          return BoardList.renderMainPageBoardList().then(function() {
            return fulfill();
          });
        };
      })(this));
    ...
    Nullchan.prototype.renderNotFound = function() {
      return this.container.innerHTML = Templates.render("not-found");
    };

    Nullchan.prototype.publishBasicContentJson = function() {
      return new Promise((function(_this) {
        return function(fulfill, reject) {
          var path;
          path = "data/users/" + _this.siteInfo.auth_address + "/test.txt";
          return _this.cmd("fileWrite", [path, btoa("fuck this")], function(write) {
            _this.log("basic write: " + write);
            return _this.cmd("siteSign", {
              inner_path: path
            }, function(basicResponse) {
              _this.log("basic responseon sign: " + (JSON.stringify(basicResponse)));
              return _this.cmd("fileGet", "data/users/" + _this.siteInfo.auth_address + "/content.json", function(contentJson) {
                return fulfill(contentJson);
              });
            });
          });
        };
          ...
          return _this.cmd("fileGet", path, function(contentJson) {
            ...
            if (!contentJson) {
              ...
              return _this.publishBasicContentJson().then(function(contentJson) {
                _this.log("Basic file created: " + contentJson + ", setting optional to it");
                return _this.setOptional(contentJson).then(function() {
                  return fulfill();
                });
              });
            } else {
              data = JSON.parse(contentJson);
              if ((data != null ? data.optional : void 0) !== ".*\\.(png|jpg|gif)") {
                _this.log("optional isn't set, setting");
                return _this.setOptional(contentJson).then(function() {
                  return fulfill();
                });
              } else {
                _this.log("optional is already set, can rest now");
    ...
    Nullchan.prototype.setOptional = function(strJson) {
      return new Promise((function(_this) {
        return function(fulfill, reject) {
          var data, json_raw, path;
          path = "data/users/" + _this.siteInfo.auth_address + "/content.json";
    ...
    Nullchan.prototype.uploadFile = function(rawBase64, fileName, publish) {
      return new Promise((function(_this) {
        return function(fulfill, reject) {
          return _this.checkOptionalSet().then(function() {
            var dir, path;
